...
export default function LeadCapture({ delayMs = 8000, repeatDays = 7 }) {
  ...
  useEffect(() => {
    ...
    const seenRecently = last && Date.now() - Number(last) < repeatDays * 24 * 60 * 60 * 1000;
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.name || !form.email || !form.consent) return alert('Please complete all fields and consent.');
    try {
      setSubmitting(true);
        ...
        {!success ? (
          <>
            ...
            <form className="space-y-3" onSubmit={onSubmit}>
              ...
              <button type="button" onClick={close} className="block w-full text-center text-xs text-gray-500 underline">
                No thanks
              </button>
            </form>
          </>
          ...
          <div className="text-center animate-fadeIn">
            ...
            <p className="text-gray-600 mt-2">Check your inbox for the resource pack. Thanks for supporting our mission!</p>
